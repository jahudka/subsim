...
export class ContextRenderer extends EventEmitter {
  ...
  private renderFrame(): void {
    ...
    this.ctx.font = '12px/1 "Source Sans Pro", sans-serif';
    this.textWidth ??= this.ctx.measureText('-00.00 dB').width;
    ...
    const minGain = -48;
    const textPad = 10;
    const w = this.width - textPad - this.textWidth;
    ...
    this.ctx.fillRect(this.textWidth + textPad, this.height / 2, w, 1);
    this.ctx.fillStyle = '#000';
    this.ctx.fillText(`${gain.toFixed(2).replace(/infinity/i, 'inf')} dB`, this.textWidth, this.height / 2);
    ...
    for (const { delay, gain } of arrivals) {
      ...
      if (t < delayRange) {
        this.ctx.fillRect(this.textWidth + textPad + w * t / delayRange, (this.height - h) / 2, 1, h);
      }
    }
