...
export const SimulationUI: FC = () => {
  ...
  const setGain = useCallback((g: number) => dispatch(set.opt('simulation', 'gain', g)), [dispatch]);
    ...
    <>
      ...
      <table>
        <tbody>
          ...
          <Row id="sim-gain" label="Gain:">
            <NumericField id="sim-gain" value={state.gain} onChange={setGain} min={-24} max={24} step={1}>dB</NumericField>
            <NumericField slider value={state.gain} onChange={setGain} min={-24} max={24} step={1} />
          </Row>
